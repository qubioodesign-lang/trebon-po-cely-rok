...
export type BranaInstalacniDiagnostikaStav =
  | "handler spuštěn"
  | "prompt nedostupný"
  | "prompt volán"
  | "instalace přijata"
...
function zalogovatBranaPwa(
  zprava: string,
  data?: unknown,
): void {
  if (data === undefined) {
    console.log(`[BRÁNA PWA] ${zprava}`);
    return;
  }

  console.log(`[BRÁNA PWA] ${zprava}`, data);
}

/**
 * Dočasná diagnostika instalace – loguje do konzole a vrací stav pro UI.
 * @internal odstranit po ověření na telefonu
 */
export async function vyvolatInstalacniDialogSDiagnostikou(
  nastavStav?: (stav: BranaInstalacniDiagnostikaStav) => void,
): Promise<BranaInstalacniDiagnostikaStav> {
  const nastav = (stav: BranaInstalacniDiagnostikaStav) => {
    nastavStav?.(stav);
  };

  zalogovatBranaPwa("hlavniKlik proběhl");
  zalogovatBranaPwa(
    "window.__branaPwaInstalacniPrompt",
    Boolean(window.__branaPwaInstalacniPrompt),
  );
  zalogovatBranaPwa(
    "window.__branaPwaVcasnyPrompt",
    Boolean(window.__branaPwaVcasnyPrompt),
  );

  const prompt = ziskatUlozenyPrompt();

  if (!prompt) {
    zalogovatBranaPwa("prompt nedostupný");
    nastav("prompt nedostupný");
    return "prompt nedostupný";
  }
  ...
  try {
    zalogovatBranaPwa("volám prompt()");
    nastav("prompt volán");
    await prompt.prompt();
    const { outcome } = await prompt.userChoice;
    zalogovatBranaPwa("userChoice.outcome", outcome);

    if (outcome === "accepted") {
      nastav("instalace přijata");
      return "instalace přijata";
    }

    nastav("instalace odmítnuta");
    return "instalace odmítnuta";
  } catch (chyba) {
    const text =
      chyba instanceof Error ? chyba.message : String(chyba ?? "neznámá chyba");
    zalogovatBranaPwa("chyba", text);
    const stav = `chyba: ${text}` as BranaInstalacniDiagnostikaStav;
    nastav(stav);
    return stav;
  }
}
