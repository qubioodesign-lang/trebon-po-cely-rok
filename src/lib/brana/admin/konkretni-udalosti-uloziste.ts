...
/** Dočasný test – jednoznačný název persistované CEKA události */
export const BRANA_TEST_PERSIST_CEKA_NAZEV =
  "TEST PERSIST – čeká na schválení";

const BRANA_TEST_PERSIST_ID_PREFIX = "test-persist-";
const BRANA_TEST_PERSIST_DATUM = "2026-10-05";

export type VytvoritTestCekajiciUdalostVysledek = {
  vytvoreno: boolean;
...
/**
 * DOČASNÁ admin-only testovací akce.
 * Vytvoří maximálně jednu persistovanou CEKA_NA_SCHVALENI pro ověření Schválit.
 * Ukázková data nemění.
 */
export async function vytvoritTestCekajiciUdalost(): Promise<VytvoritTestCekajiciUdalostVysledek> {
  if (!(await jeAdminPrihlasen())) {
    throw new Error("Nejste přihlášeni.");
  }
  ...
  if (!maBranaAdminBlobKonfiguraci()) {
    throw new Error(
      "Nelze vytvořit testovací událost: chybí BLOB_BRANA_ADMIN_STORE_ID nebo BLOB_BRANA_ADMIN_READ_WRITE_TOKEN.",
    );
  }

  const dokument = await nacistDokumentProZapis();

  const existujici = dokument.udalosti.find(
    (u) =>
      u.nazev === BRANA_TEST_PERSIST_CEKA_NAZEV ||
      u.id.startsWith(BRANA_TEST_PERSIST_ID_PREFIX),
  );
  if (existujici) {
    return {
      vytvoreno: false,
      udalost: existujici,
      zprava: "Testovací čekající událost už existuje.",
    };
  }

  const nova: BranaKonkretniUdalost = {
    id: `${BRANA_TEST_PERSIST_ID_PREFIX}${crypto.randomUUID()}`,
    redakcniPolozkaId: "divadlo-jk-tyla",
    datumOd: BRANA_TEST_PERSIST_DATUM,
    datumDo: BRANA_TEST_PERSIST_DATUM,
    cas: "12:30",
    mistoNeboTyp: "Testovací místo",
    nazev: BRANA_TEST_PERSIST_CEKA_NAZEV,
    rucniPoziceVDni: null,
    stavSchvaleni: "CEKA_NA_SCHVALENI",
  };

  const vysledny = parsovatDokument({
    ...dokument,
    udalosti: [...dokument.udalosti, nova],
  });
  if (!vysledny) {
    throw new Error("Výsledný dokument neprošel validací. Nic nebylo uloženo.");
  }
  ...
  return {
    vytvoreno: true,
    udalost: nova,
    zprava: "Testovací čekající událost vytvořena.",
  };
}
