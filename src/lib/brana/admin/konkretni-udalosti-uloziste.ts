...
} from "./uklid-galerie-105-vystavy";
 ...
 * Stejný filtr jako preview (`sestavPreviewUklidGalerie105Vystavy`).
 * Fail-closed: count !== 19 → žádný zápis.
 * Po dokončení úklidu odstranit spolu s preview stránkou.
 */
export async function vyrazitGalerie105VystavyCekaJednorazove(): Promise<{
  zmeneno: number;
  ids: string[];
  ...
  if (!maBranaAdminBlobKonfiguraci()) {
    throw new Error(
      "Nelze vyřadit výstavy Galerie 105: chybí BLOB_BRANA_ADMIN_STORE_ID nebo BLOB_BRANA_ADMIN_READ_WRITE_TOKEN.",
    );
  }

  const dokument = await nacistDokumentProZapis();
  const preview = sestavPreviewUklidGalerie105Vystavy(dokument.udalosti);
  if (!preview.ok) {
    throw new Error(
      `STOP: počet výstav k úklidu je ${preview.skutecnyPocet}, očekáváno ${BRANA_UKLID_GALERIE_105_VYSTAV_OCEKAVANY_POCET}. Nic nebylo uloženo.`,
    );
  }

  const akceCekaPred = dokument.udalosti.filter(
    (u) =>
  ...
  const idSet = new Set(preview.vybrano.map((u) => u.id));
  const scanKlicPred = new Map<string, string>();
  for (const u of dokument.udalosti) {
    if (idSet.has(u.id) && typeof u.scanKlic === "string") {
      scanKlicPred.set(u.id, u.scanKlic);
    }
  }

  const noveUdalosti = dokument.udalosti.map((u) => {
    if (!idSet.has(u.id)) {
      return u;
    }
    return {
      ...u,
      rucniPoziceVDni: null,
      stavSchvaleni: "VYRAZENO" as const,
    };
  });
  dokument.udalosti = noveUdalosti;
  ...
  const cekaVystavyPo = vybratGalerie105VystavyCekaKUklidu(
    overeni.udalosti,
  ).length;
  const vyrazenoVystavyPo = overeni.udalosti.filter(
    (u) =>
      idSet.has(u.id) &&
      u.stavSchvaleni === "VYRAZENO" &&
      u.redakcniPolozkaId === "galerie-105",
  ).length;
  const akceCekaPo = overeni.udalosti.filter(
    (u) =>
      u.redakcniPolozkaId === "galerie-105" &&
      u.stavSchvaleni === "CEKA_NA_SCHVALENI" &&
      typeof u.scanKlic === "string" &&
  ...
  let scanKlicZachovan = true;
  for (const u of overeni.udalosti) {
    if (!idSet.has(u.id)) {
      continue;
    }
    const pred = scanKlicPred.get(u.id);
    if (
      typeof pred !== "string" ||
      typeof u.scanKlic !== "string" ||
      u.scanKlic !== pred
    ) {
      scanKlicZachovan = false;
      break;
    }
  }
  ...
  if (
    cekaVystavyPo !== 0 ||
    vyrazenoVystavyPo !== BRANA_UKLID_GALERIE_105_VYSTAV_OCEKAVANY_POCET ||
    akceCekaPo !== akceCekaPred ||
    !scanKlicZachovan
  ) {
    throw new Error(
      `Úklid zapsán, ale následná kontrola selhala (cekaVystavyPo=${cekaVystavyPo}, vyrazeno=${vyrazenoVystavyPo}, akcePred=${akceCekaPred}, akcePo=${akceCekaPo}, scanKlic=${scanKlicZachovan}).`,
    );
  }

  return {
    zmeneno: BRANA_UKLID_GALERIE_105_VYSTAV_OCEKAVANY_POCET,
    ids: preview.vybrano.map((u) => u.id),
    cekaVystavyPo,
    vyrazenoVystavyPo,
    akceCekaPred,
    akceCekaPo,
