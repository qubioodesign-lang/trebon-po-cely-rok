/**
 * CAS / ifMatch smyčka pro RMW zápisy data/brana-radar.json.
 * Samostatný obal — nesdílí kalendářní, zdrojové, upozornění ani nezařazené mutátory.
 * Mutator = operace na čerstvém dokumentu, ne snapshot.
 */

export const BRANA_RADAR_CAS_MAX_POKUSU = 8;

export const BRANA_RADAR_CAS_CHYBA_KONFLIKT =
  "RADAR mezitím opakovaně změnil jiný zápis. Změna se neuložila.";

export class BranaRadarCasKonfliktLimitError extends Error {
  constructor() {
    super(BRANA_RADAR_CAS_CHYBA_KONFLIKT);
    this.name = "BranaRadarCasKonfliktLimitError";
  }
}

export type BranaRadarDokumentMutace<TDokument, TVysledek> =
  | { typ: "zapsat"; dokument: TDokument; vysledek: TVysledek }
  | { typ: "bezZmeny"; vysledek: TVysledek };

export type BranaRadarCasCteni<TDokument> =
  | { stav: "neexistuje" }
  | { stav: "ok"; dokument: TDokument; etag: string };

export type BranaRadarCasIo<TDokument> = {
  nacist: () => Promise<BranaRadarCasCteni<TDokument>>;
 ...
 */
export async function zmenitRadarDokumentAtomickySIo<TDokument, TVysledek>(
  io: BranaRadarCasIo<TDokument>,
  mutator: (
    dokument: TDokument,
  ) => BranaRadarDokumentMutace<TDokument, TVysledek>,
  maxPokusy: number = BRANA_RADAR_CAS_MAX_POKUSU,
): Promise<TVysledek> {
  ...
  throw new BranaRadarCasKonfliktLimitError();
}
