/**
 * CAS / ifMatch smyčka pro RMW zápisy data/brana-redakcni-poradi.json.
 * Samostatný obal — nesdílí kalendářní, zdrojové, upozornění, nezařazené ani RADAR mutátory.
 * Mutator = operace na čerstvém dokumentu, ne snapshot.
 */

export const BRANA_REDAKCNI_CAS_MAX_POKUSU = 8;

export const BRANA_REDAKCNI_CAS_CHYBA_KONFLIKT =
  "Redakční pořadí mezitím opakovaně změnil jiný zápis. Změna se neuložila.";

export class BranaRedakcniCasKonfliktLimitError extends Error {
  constructor() {
    super(BRANA_REDAKCNI_CAS_CHYBA_KONFLIKT);
    this.name = "BranaRedakcniCasKonfliktLimitError";
  }
}

export type BranaRedakcniDokumentMutace<TDokument, TVysledek> =
  | { typ: "zapsat"; dokument: TDokument; vysledek: TVysledek }
  | { typ: "bezZmeny"; vysledek: TVysledek };

export type BranaRedakcniCasCteni<TDokument> =
  | { stav: "neexistuje" }
  | { stav: "ok"; dokument: TDokument; etag: string };

export type BranaRedakcniCasIo<TDokument> = {
  nacist: () => Promise<BranaRedakcniCasCteni<TDokument>>;
 ...
 * Field conflict uvnitř mutátoru není 412 a neretriuje se.
 */
export async function zmenitRedakcniPoradiDokumentAtomickySIo<
  TDokument,
  TVysledek,
>(
  io: BranaRedakcniCasIo<TDokument>,
  mutator: (
    dokument: TDokument,
  ) => BranaRedakcniDokumentMutace<TDokument, TVysledek>,
  maxPokusy: number = BRANA_REDAKCNI_CAS_MAX_POKUSU,
): Promise<TVysledek> {
  ...
  throw new BranaRedakcniCasKonfliktLimitError();
}
