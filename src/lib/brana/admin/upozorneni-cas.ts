/**
 * CAS / ifMatch smyčka pro RMW zápisy data/brana-upozorneni-nastaveni.json.
 * Samostatný obal — nesdílí kalendářní ani zdrojové mutátory.
 * Mutator = operace na čerstvém dokumentu, ne snapshot.
 */

export const BRANA_UPOZORNENI_CAS_MAX_POKUSU = 8;

export const BRANA_UPOZORNENI_CAS_CHYBA_KONFLIKT =
  "Nastavení upozornění mezitím opakovaně změnil jiný zápis. Změna se neuložila.";

export class BranaUpozorneniCasKonfliktLimitError extends Error {
  constructor() {
    super(BRANA_UPOZORNENI_CAS_CHYBA_KONFLIKT);
    this.name = "BranaUpozorneniCasKonfliktLimitError";
  }
}

export type BranaUpozorneniDokumentMutace<TDokument, TVysledek> =
  | { typ: "zapsat"; dokument: TDokument; vysledek: TVysledek }
  | { typ: "bezZmeny"; vysledek: TVysledek };

export type BranaUpozorneniCasCteni<TDokument> =
  | { stav: "neexistuje" }
  | { stav: "ok"; dokument: TDokument; etag: string };

export type BranaUpozorneniCasIo<TDokument> = {
  nacist: () => Promise<BranaUpozorneniCasCteni<TDokument>>;
 ...
 */
export async function zmenitUpozorneniDokumentAtomickySIo<TDokument, TVysledek>(
  io: BranaUpozorneniCasIo<TDokument>,
  mutator: (
    dokument: TDokument,
  ) => BranaUpozorneniDokumentMutace<TDokument, TVysledek>,
  maxPokusy: number = BRANA_UPOZORNENI_CAS_MAX_POKUSU,
): Promise<TVysledek> {
  ...
  throw new BranaUpozorneniCasKonfliktLimitError();
}
